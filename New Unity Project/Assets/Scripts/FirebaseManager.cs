    ...
    private IEnumerator Register(string _email, string _password, string _username)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                var DBTask = DBreference.Child("users").Child(User.UserId).Child("score1").SetValueAsync(0);
                ...
                var DB1Task = DBreference.Child("users").Child(User.UserId).Child("score2").SetValueAsync(0);
                ...
                var DB2Task = DBreference.Child("users").Child(User.UserId).Child("score3").SetValueAsync(0);
                ...
                var DB3Task = DBreference.Child("users").Child(User.UserId).Child("scoref").SetValueAsync(0);

                yield return new WaitUntil(predicate: () => DB3Task.IsCompleted);

                if (DB3Task.Exception != null)
                {
                    Debug.LogWarning(message: $"Failed to register task with {DBTask.Exception}");
                }
               
                var DB4Task = DBreference.Child("users").Child(User.UserId).Child("username").SetValueAsync(User.DisplayName);

                yield return new WaitUntil(predicate: () => DB4Task.IsCompleted);
            }
        }
    }
